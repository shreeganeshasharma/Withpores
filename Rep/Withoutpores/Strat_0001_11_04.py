...
for i in range(nump):
  ...
  for j in range(1, nums):
    ...
    elif R < np.linalg.norm(X[:, j - 1]) < R1 and R1 < np.linalg.norm(X[:, j - 1] + dX1) < R2:
    ...
    elif R1 < np.linalg.norm(X[:, j - 1]) < R2 and R < np.linalg.norm(X[:, j - 1] + dX2) < R1:
      #Find the point of diffusion
...
print("Strat nump = ", nump, "nums = ", nums, "dt = ", dt, "D1 = ", D1, " D2 = ", D2, "--- %s seconds ---" % (time.time() - start_time))
...
plt.legend()
plt.savefig(f'Images/Hist_Str_{R}_{nump}_{nums}_{D1}_{D2}.png')
#plt.show()
...
plt.savefig(f'Images/Prob_Str_{R}_{nump}_{nums}_{D1}_{D2}.png')
#plt.show()
...
plt.ylabel("Time, t")
plt.colorbar()
plt.savefig(f'Images/Heat_Str_{R}_{nump}_{nums}_{D1}_{D2}.png')
