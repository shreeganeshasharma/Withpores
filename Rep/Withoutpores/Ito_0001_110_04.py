...
for i in range(nump):
  ...
  for j in range(1, nums):
    ...
    elif R < np.linalg.norm(X[:, j - 1]) < R1 and R < np.linalg.norm(X[:, j - 1] + dX1) < R2:
      X[:, j] = X[:, j - 1] + dX1

    #Particle is currently in medium2, (almost) never hits the target directly from D2, might stay in D2 or diffuse to D1
    elif R1 < np.linalg.norm(X[:, j - 1]) < R2 and R < np.linalg.norm(X[:, j - 1] + dX2) < R2:
      X[:, j] = X[:, j - 1] + dX2
...
plt.legend()
plt.savefig(f'Images/Hist_Ito_{R}_{nump}_{nums}_{D1}_{D2}.png')
#plt.show()
...
plt.savefig(f'Images/Prob_Ito_{R}_{nump}_{nums}_{D1}_{D2}.png')
#plt.show()
...
plt.ylabel("Time, t")
plt.colorbar()
plt.savefig(f'Images/Heat_Ito_{R}_{nump}_{nums}_{D1}_{D2}.png')
